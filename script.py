...
while (tick<=10): # loop by default will gather data for 5 ticks, tick=15seconds
    ...
    data.append([Timestamp,cpu, memory.percent, disk.percent]) #appends all data to list

    tick = tick +1 #incrementing tick
    time.sleep(1) #Pause

df1 = pd.DataFrame(data,columns=['Timestamp','Cpu Usage (%)','Memory Usage (%)','Disk usage (%)'])
